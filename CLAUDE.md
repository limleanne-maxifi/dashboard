...
- **v4.7 (2026-08-09): `/about` founder bio, second paragraph rewritten — SEO-does-not-transfer insight now leads the paragraph, and the technology-consultancy role moved from past tense ("Previously, she led") to present ("She now leads"), with the client set widened from "law firms" to "B2B expert-led organisations, professional associations, and industry bodies". Copy supplied verbatim by Le-Anne 9 Aug 2026. Only `src/pages/about.astro` carries this bio — the Person/Organization/FAQ JSON-LD on `/about` does not repeat it. `docs/aeo-fixes/pages/about.html` holds a different, older Webflow-era founder bio and was intentionally left unchanged.**
