...
- **v4.7 (2026-08-09): `/about` founder bio, second paragraph rewritten. The SEO-does-not-transfer insight now leads the paragraph; the credential is "background in communications, PR and marketing strategy"; and the client work (B2B expert-led organisations, professional associations, industry bodies — widened from "law firms") is attributed to Maxifi Digital directly. **The unnamed "Singapore-based technology consultancy" is removed and must not return** — Maxifi Digital is the only agency Lim Le-Anne has led, and a second unnamed Singapore consultancy on the same page created an entity-disambiguation problem for the engines this site is optimised for. Her earlier PR-agency experience is background, not a concurrent role. Consequently the `/about` Person JSON-LD keeps a **single** `worksFor` (Maxifi Digital) — do not make it an array. Copy supplied verbatim by Le-Anne 9 Aug 2026. Only `src/pages/about.astro` carries this bio — the Person/Organization/FAQ JSON-LD does not repeat it. `docs/aeo-fixes/pages/about.html` was re-synced to match: its Webflow-era founder bio claimed "a senior communications background spanning Singapore's public sector and industry", which is **not** the approved credential — the public-sector claim is retired and must not be reintroduced.**
- **Founder bio — single source of truth (v4.7):** the live bio lives in `src/pages/about.astro` only. `docs/aeo-fixes/pages/about.html` is a stale Webflow-migration artifact that does not ship; if the bio changes again, update `about.astro` first and re-sync that file from it, never the reverse. Three claims are locked: the credential is "communications, PR and marketing strategy" (no public-sector framing); client work is attributed to Maxifi Digital directly; and no second consultancy — named or unnamed — appears anywhere on `/about`.
